...
import excel_translate_name
# import selenium_translate_name
import requests
...
def plus():  # 主呼叫程式，輸入並按下BUTTON後執行
    leg_name = []
    leg = excel_translate_name.translate_leg_name()
    headers = {
    ...
    url = f"https://tw.op.gg/modes/aram"
    ...
    t = 0
    for i in leg_name:  # 從所有中文英雄名裡找
        if input_leg_name in i:  # 如果輸入的值有在完整中文英雄名裡
...
def get_inf(name):  # 利用英文名進行爬蟲，回傳貼上福文
    ...
    url = f"https://www.op.gg/modes/aram/{name}/build"
    resp = requests.get(url, headers=headers)
    soup = BeautifulSoup(resp.text, "html.parser")
    row_data = []

    big_con = soup.select(".item_mark img")[0]["alt"]  # 左主1
